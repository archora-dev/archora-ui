...
describe("foundation exports", () => {
  ...
  it("persists custom theme names", () => {
    const theme = useTheme();
    theme.setTheme("brand");

    expect(document.documentElement.dataset.theme).toBe("brand");
    expect(window.localStorage.getItem("archora-ui-theme")).toBe("brand");
    expect(theme.preference.value).toBe("brand");
  });
