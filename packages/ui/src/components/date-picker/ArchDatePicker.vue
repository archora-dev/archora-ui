...
const fallbackMinYear = visibleMonth.value.year - 100;
const fallbackMaxYear = visibleMonth.value.year + 100;
...
const yearOptions = computed(() => {
  const minYear = props.min ? Number(props.min.slice(0, 4)) : fallbackMinYear;
  const maxYear = props.max ? Number(props.max.slice(0, 4)) : fallbackMaxYear;
  const years: number[] = [];
...
function setVisibleMonth(month: number) {
  visibleMonth.value = {
    year: visibleMonth.value.year,
    month
  };
}
...
function selectDay(day: ArchDatePickerDay) {
  ...
  const newDate = String(day.date);
  emit("update:modelValue", newDate);
  emit("change", newDate);
  open.value = false;
}

useOutsideClick([rootRef, popoverRef], () => {
  open.value = false;
});
    ...
    <Teleport to="body">
      ...
      >
        <header class="arch-date-picker__header">
          ...
          <div class="arch-date-picker__selectors" :aria-label="monthLabel">
            <select
              class="arch-date-picker__select"
              aria-label="Month"
              :value="visibleMonth.month"
              @change="setVisibleMonth(Number(($event.target as HTMLSelectElement).value))"
            >
            ...
            <select
              class="arch-date-picker__select arch-date-picker__select--year"
              aria-label="Year"
              :value="visibleMonth.year"
              @change="setVisibleYear(Number(($event.target as HTMLSelectElement).value))"
            >
